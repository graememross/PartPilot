...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (existing.warehouse.ownerId !== userId) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    ...
    export async function DELETE(request: NextRequest, { params }: { params: { id: string, order: string } }) {
    try {
        const id = params.id;
